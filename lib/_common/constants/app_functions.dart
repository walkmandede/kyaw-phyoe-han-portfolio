...
void saveLogFromException(e1,e2){
  Get.find<LoggerController>().insertNewLog(log: LoggerModel.fromExceptions(title: e1.toString(), stackTrace: e2));
}
...
class AppFunctions {
  ...
  static void pushTo(dynamic page) {
  ...
  dynamic parseEnum<T>({
    ...
    try {
      ...
      Get.find<LoggerController>().insertNewLog(log: LoggerModel.fromExceptions(
          title: e1.toString(), stackTrace: e2));
      null;
    }
    return result;
  }
      ...
      try {
        if (firstDate.year == lastDate.year) {
          ...
          if (firstDate.month == lastDate.month) {
            ...
            dateString =
            "${DateFormat("d MMM").format(firstDate)}-${DateFormat("d MMM")
                .format(lastDate)}";
          }
        } else {
          //notSameYear
          dateString =
          "${DateFormat.yMMMd().format(firstDate)}-${DateFormat.yMMMd().format(
              lastDate)}";
        }
      } catch (e1, e2) {
        Get.find<LoggerController>().insertNewLog(
            log: LoggerModel.fromExceptions(
                title: e1.toString(), stackTrace: e2));
      }
      return dateString;
    }
